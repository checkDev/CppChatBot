...
ChatLogic::ChatLogic()
{
    ...
    //  move chatbot. So set it as root node
...
ChatLogic::~ChatLogic()
{
    ...
    // delete all nodes
    // unique_ptr will be deleted automatically so manually deleting not needed
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // GraphNode * converted to unique_ptr referrence
                            ...
                            // create new edge
                            // make the edge unique
                            std::unique_ptr<GraphEdge> edge = std::make_unique<GraphEdge>(id);
                            // Get the node pointers
                            edge->SetChildNode(childNode->get());
                            edge->SetParentNode(parentNode->get());
                            // _edges.push_back(std::move(edge)); //  not needed here

                            // find all keywords for current node
                            AddAllTokensToElement("KEYWORD", tokens, *edge);

                            // store reference in child node and parent node
                            (*childNode)->AddEdgeToParentNode(edge.get()); //  get() because its a unique_ptr
                            (*parentNode)->AddEdgeToChildNode(std::move(edge)); //  move the edge for ownership to childNodes
                        }
                    }
                }
                else
